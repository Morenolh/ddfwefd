...
AddEventHandler('TireEntendu', function(gx, gy, gz)
     ...
     if PlayerData.job ~= nil and PlayerData.job.name == 'police' then
          ...
          ESX.ShowAdvancedNotification(
               ...
               'Llamada ciudadana sobre disparos ~g~Y~s~ Para atender la llamada ~r~X~s~ Rechazar.', 'CHAR_CALL911', 8)
...
AddEventHandler('PriseAppel', function(name)
     ...
     if PlayerData.job ~= nil and PlayerData.job.name == 'police' then
          ...
          ESX.ShowNotification('~w~El agente ~g~'..name..'~w~ tomó la llamada')
...
Citizen.CreateThread(function()
     ...
     while true do
          ...
          elseif IsControlJustPressed(1, 77) and alerteEnCours then

               AlertePrise = false

               alerteEnCours = false

               --ESX.ShowNotification('~w~Se ha mandado un entorno!')
